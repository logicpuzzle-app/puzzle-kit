...
export default defineConfig({
  ...base,
  testMatch: ['**/build-entrypoints.spec.ts', '**/number-history.spec.ts', '**/cursor-style.spec.ts', '**/persistence.spec.ts', '**/npgen.spec.ts', '**/solver.spec.ts', '**/symbol-sizing-production.spec.ts', '**/special-export.spec.ts', '**/text-editing.spec.ts', '**/number-pad-history.spec.ts', '**/special-touch.chromium-touch.spec.ts', '**/puzzle-session.spec.ts'],
  grep: /session:|special-touch:|number-pad:|text-ui:|text-touch:|png:|symbol-ui:|build:|directional number insertion|marker keys|selection color|autosave survives|generates a seeded|solver:/,
