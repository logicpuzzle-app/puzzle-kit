...
import { resolveLinePoints, type ResolvedBoardPoint } from './lineReferences';
...
export function mergeLineOverlaps(
  ...
  grid: GridConfig, topology: GridTopology | null, groups: Record<string, LineGroup> = {}, useTopology = !!topology,
  ...
  const points = resolveLinePoints(incoming, context);
  if (!points) return { line: incoming, removed: [] };
  const [startRef, endRef] = points;
  const start = startRef.position, end = endRef.position;
  ...
  const target = (line: LineElement) => line.lineTarget ?? (resolveLinePoints(line, context)?.every(p => p.type === 'vertex') ? 'edge' : 'cell');
  let low = 0, high = length;
  let from: ResolvedBoardPoint = startRef, to: ResolvedBoardPoint = endRef;
  ...
  while (changed) {
    ...
    for (const line of remaining) {
      ...
      const pair = resolveLinePoints(line, context);
      if (!pair) continue;
      const [ar, br] = pair, a = ar.position, b = br.position;
      ...
      if (min < low) { low = min;from = pa < pb ? ar : br; }
      if (max > high) { high = max;to = pa < pb ? br : ar; }
    }
  }
  ...
  for (const line of removed) {
    const [ar, br] = resolveLinePoints(line, context)!;
    const a = ar.position, b = br.position;
    if (Math.abs(Math.min(project(a), project(b)) - low) < 1e-6 && Math.abs(Math.max(project(a), project(b)) - high) < 1e-6) return { line, removed };
  }
  const [normFrom, normTo] = normalizeSegmentEndpoints(from.id, to.id);
  if (low === 0 && high === length) return { line: incoming, removed };
  const { edgeId: _edgeId, ...rest } = incoming;
  if (incoming.fromType || incoming.toType || removed.some(line => line.fromType || line.toType)) {
    return { line: { ...rest, from: from.id, to: to.id, fromType: from.type, toType: to.type }, removed };
  }
...
export function normalizeLineOverlaps(lines: Record<string, LineElement>, grid: GridConfig, topology: GridTopology | null, groups: Record<string, LineGroup> = {}, useTopology = !!topology) {
  const normalized: Record<string, LineElement> = {};
  for (const line of Object.values(lines)) {
    const merged = mergeLineOverlaps(line, normalized, grid, topology, groups, useTopology);
