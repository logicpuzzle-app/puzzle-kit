...
or custom labels, diagonal mode, the Java-compatible seed, solver/uniqueness
bitmasks, difficulty bounds, and the forbidden number. Long-running calls
should be made from a Web Worker.

`solve`, `generate`, and `random` accept `--use <list>`, `--unique <list>`,
`--dp-min N`, and `--dp-max N`. Technique names are `localization`,
`naked-pair`, `hidden-pair`, `naked-triple`, `hidden-triple`, `x-wing`, and
`swordfish`; uniqueness names are `vh`, `cell`, and `block`. Omitting either
list enables all of its entries. Difficulty bounds are inclusive and are
...
`--blocks @file.txt`, and `--diagonal`. A block specification is required for
non-square sizes. Text grids use `N` rows of `N` whitespace-separated cells.
...
`.xml` are detected automatically on input.
