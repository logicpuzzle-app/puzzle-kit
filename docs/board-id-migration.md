...
| [汎用線入力・端点参照](line-input-identity.md) | 新しい入力は端点の種類を保持し、明示モードで実接続を解決する。線レコードIDに依存せず消去し、混合端点と矢印方向を保存する | 型なしの旧データは辺参照または一意な点から解決し、曖昧な参照を推測しない。参照モード切替時の既存線移行、公開互換APIや他ジャンルのID解析の整理は継続 |
...
既存データの参照変換・モードを含むUndo／Redoを行っていない。
Gridで作った頂点線がTopologyへの切替で未解決となることを、
実ストアと描画用resolverで確認済み。画面経由のQAは未実施。

移行は[参照モード切替の契約](board-id-contract.md#参照モードの切替はデータ移行として扱う)に従う。
線だけを変換せず、数字・塗り・記号・部屋・試行・設定等の参照範囲を調べてから一体で適用する。
