...
2026-09-06、branch `feature/touch-browser-quality`、検証対象HEAD `f59662f`。前提はPR #49の712ccc3。Playwright 1.62.0、Vitest 4.1.11、ローカルNode v25.2.1 / npm 11.7.0。
...
| --- | --- |
| アプリ型検査 / E2E型検査 | PASS |
| Unit / integration | 60 files / 985 tests PASS |
| E2E | 116 PASS / 8 SKIP / 0 FAIL |
| 本番build | PASS |
...
全検査ログ・E2E動画・traceは `artifacts/check/2026-09-06T02-46-19-518Z`。既存solverのsource map不足とbuildの大きなchunk警告は残る。CIは独立したLinux/Node 24環境で同じ検査を実行し、結果はPR checksで確認する。
...
[6フロー・12本の比較動画](evidence-touch-webkit-20260906/README.md) を同梱。比較ページの全動画の読み込みを実ブラウザーで確認した。
